/*
	...
	And can minimize network traffic between server and client
	On client:
	Client is mostly agnostic to what is being listed and bought
	All it needs to know is how to display lists of items and handle their purchase
*/
...
NWG_MSHOP_CLI_Settings =  createHashMapFromArray [
	...
	/*Player money text*/
	["EXPECT_PLAYER_MONEY_TEXT_PREFILLED",true],
...
NWG_MSHOP_CLI_OpenShopTab = {
	...
	for "_i" from 0 to 100 do {
		...
		private _itemName = (format [_locTemplate,_categoryIndex,_i]) call NWG_fnc_localize;
		private _itemPrice = NWG_MSHOP_CLI_priceMap get _itemName;
		private _itemRow = format [(NWG_MSHOP_CLI_Settings get "ITEM_ROW_TEMPLATE"),_itemName,_itemPrice];
...
NWG_MSHOP_CLI_OnMapClick = {
	...
	switch (true) do {
		...
		case (NWG_MSHOP_CLI_selectedItem isNotEqualTo ""): {
			...
			//Send request to server
			[player,_itemName,_pos] remoteExec ["NWG_fnc_mshopItemRequest",2];
		...
		case (NWG_MSHOP_CLI_selectedVehicle isNotEqualTo ""): {
			...
			//Send request to server
			[player,_vehicleClassname,_pos] remoteExec ["NWG_fnc_mshopVehicleRequest",2];

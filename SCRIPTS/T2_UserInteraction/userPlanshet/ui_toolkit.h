/*Copy of ui_toolkit.hpp to be used in 03Group.sqf*/
...
// ----- Interface size & resolution ----- (Removed: Asmo)
...
// Determine if UI & text scales are relative to UI size
	...
	// Replaced: Asmo. It was causing UI on lower resolutions to go off the screen, so for me it is better to have a smaller UI but visible.
	#define UI_SCALE 1 // Constant UI scale across all UI sizes
	#define TEXT_SCALE_UI 1 // Regular text scale
#else
	#define UI_SCALE 1 // Regular UI scale
	// #define TEXT_SCALE_UI (UI_SIZE_DEV / UI_SIZE) // (Same as above: Asmo)
	#define TEXT_SCALE_UI 1 // Compensate text scale against UI size
#endif
...
// Offsets (removed: Asmo. Why the hell we need to offset by 0?)
// #define X_OFFSET 0
// #define Y_OFFSET 0
...
// Positions relative to top left, plus above offsets (Replaced: Asmo)
#define X_POS(VALUE) (SZ_LEFT + (VALUE * X_SCALE)) // stretches X value to full screen width
#define Y_POS(VALUE) (SZ_TOP + (VALUE * Y_SCALE)) // stretches Y value to full screen height
...
// Added: Asmo.
#define FROM_CENTER(CHILD_SIZE) (0.5 - ((CHILD_SIZE) / 2))